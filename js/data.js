...
(function (root) {
  ...
  // 圣火是唯一主光：约 2300K 琥珀（sRGB），按距离做指数衰减，没有光锥硬边。
  // 环境光压得很低，只留阴影层次，颜色是约 10000K 的靛蓝（绿通道不高于红）。
  // band：固有色从「圈内更深的绿褐、饱和」收到「圈外近乎单色的冷蓝」。inner/outer 是圣域半径的倍数，smoothstep，很宽。
  // greenKill：圈内把过亮的绿草收到褐绿。fog 在圈边才开始变浓，近火是暖雾，远处是深靛蓝。
  // enemy：圈外的敌人立牌再压暗、去饱和。bloom 用已经在跑的泛光通道，阈值放低、强度很小，只晕开火。
  // veil：旧的圣域黑盘，关掉，避免再切出一圈硬边。
  RW.C1_LIGHT = {
    flame: { color: [1.0, 0.545, 0.165], fall: 1.4, gain: [1.25, 1.85] },
    band: { inner: 0.2, outer: 1.7 },
    albedo: { inSat: 1.12, outSat: 0.06, inGain: 0.74, outGain: [0.62, 0.46], greenKill: [0.78, 0.25] },
    cool: [0.46, 0.40, 1.06],
    fog: { warm: [1.0, 0.50, 0.16], start: 0.78, thick: 1.05, amount: [0.42, 0.58] },
    grade: { shadow: 0.62, high: 0.4, shad: [0.10, 0.08, 0.26], highCol: [1.12, 0.58, 0.16] },
    enemy: { gain: 0.42 },
    bloom: { thr: 0.64, radius: 0.62, add: 0.1 },
    exposure: 1.06,
    veil: 0
  };
