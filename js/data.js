...
(function (root) {
  ...
  RW.AD = {
    ...
    REWARD_REROLL: '',   // 激励视频：整备免费刷新一次（第 4 波起）
    FIRST_AD_WAVE: 4,    // 前三波不出现任何广告入口
    // 以下只约束 crazygames 适配器。桌面版不播广告、重燃不限次；微信路径不读这些上限。
    REVIVE_PER_SESSION: 1,   // 激励复活：一次打开游戏只给 1 次
    REROLL_PER_RUN: 1,       // 激励刷新：每局 1 次（金币刷新不限，价格逐次上升）
    DOUBLE_PER_RUN: 1,       // 结算翻倍：每局 1 次广告，也可以用局外金币买
    MIDGAME_SKIP_RUNS: 2,    // 本会话前 2 局结束时不请求中插
    MIDGAME_GAP: 180,        // 本地至少隔这么多秒才再请求中插；SDK 还可以更稀
    REVIVE_GOLD: 15,         // 激励次数用完后，用局内金币重燃一次
    DOUBLE_GOLD_RATE: 0.4,   // 金币翻倍的花费 = max(下限, 本局金币 × 比例)
    DOUBLE_GOLD_MIN: 6
  };
