...
(function (root) {
  ...
  // 圣火是主光：金橙（少红、多黄），按距离做指数衰减，没有光锥硬边。
  // 圈外是能看清村屋剪影的蓝灰，不是发闷的品红。cool 的红低于绿、蓝最高。
  ...
  RW.C1_LIGHT = {
    // gain 约是上一档 [1.25, 1.85] 的 1.3 倍。fall 仍是软指数。
    flame: { color: [1.0, 0.74, 0.32], fall: 1.35, gain: [1.63, 2.4] },
    band: { inner: 0.2, outer: 1.7 },
    // 圈外目标大约：白天远处地面 (45,60,95)，夜晚 (28,38,70)。
    albedo: { inSat: 1.08, outSat: 0.16, inGain: 0.96, outGain: [0.92, 1.05], greenKill: [0.7, 0.18] },
    cool: [0.42, 0.56, 0.88],
    fog: { warm: [1.0, 0.74, 0.34], start: 0.95, thick: 1.5, amount: [0.22, 0.20] },
    grade: { shadow: 0.30, high: 0.42, shad: [0.20, 0.28, 0.48], highCol: [1.04, 0.84, 0.4] },
    enemy: { gain: 0.66 },
