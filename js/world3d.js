...
(function (root) {
  ...
  function buildModels() {
    ...
    M.hat_bandana = model(function (g) {
      var wrap = hex('#49315f'), cloth = hex('#9b72ce'), edge = hex('#cfb0f3');
      g.box(0, 27.6, 0, 7.6, 2.2, 7.6, wrap);
      g.box(1.2, 23.2, 0, 5.2, 2.6, 7.4, hex('#1c1226'));
      // 结与双尾伸到后脑轮廓外，避免与深色头发、斗篷融在一起。
      g.box(-5.1, 28.2, 0, 1.8, 1.8, 2.8, cloth);
      g.box(-7.1, 27.9, 1.7, 4.4, 1.4, 1.5, cloth);
      g.box(-7.1, 27.9, -1.7, 4.4, 1.4, 1.5, cloth);
      g.box(-9.1, 27.8, 1.7, 1.7, 1.1, 1.5, edge);
      g.box(-9.1, 27.8, -1.7, 1.7, 1.1, 1.5, edge);
    });
    M.hat_goggles = model(function (g) {
      var leather = hex('#a9753c'), brass = hex('#dfaa58');
      g.box(0, 27.2, 0, 7.6, 1.4, 7.6, hex('#5a3a22'));
      // 镜带沿两侧绕到后脑，后扣从斜后方仍有清晰的暖色识别点。
      g.box(-0.5, 28.5, 3.9, 8.6, 1.0, 0.9, leather);
      g.box(-0.5, 28.5, -3.9, 8.6, 1.0, 0.9, leather);
      g.box(-5.3, 28.5, 0, 1.0, 1.2, 6.8, leather);
      g.box(-5.9, 28.5, 0, 0.7, 1.6, 1.8, brass);
      g.box(3.3, 28.5, 2.9, 1.6, 1.0, 1.4, leather);
      g.box(3.3, 28.5, -2.9, 1.6, 1.0, 1.4, leather);
