...
(async () => {
  ...
  const songs = await page.evaluate(() => RW.SONGS);
  let bad = 0;
  for (const song of songs) {
    const res = await page.evaluate(async ([song, secs]) => {
      const b = await RW.renderMusic(song, secs, 44100);
    ...
    fs.writeFileSync(path.join(out, song + '.wav'), wav([res.L, res.R], 44100));
    const db = x => (20 * Math.log10(Math.max(1e-9, x))).toFixed(1);
    const warn = res.peak > 0.99 ? '  ⚠ 可能削波' : (res.rms < 0.01 ? '  ⚠ 太安静' : '');
    if (warn) bad++;
    console.log(`${song.padEnd(8)} 峰值 ${db(res.peak)} dBFS  响度 ${db(res.rms)} dBFS${warn}`);
  }
