...
(async () => {
  ...
  await page.evaluate(() => { RW.game.prog.marks = 10; });
  await press('courtyard');
  await page.waitForTimeout(200);
  await shot('f7_courtyard');
  await press('yard:ward');
  const yardLevel = await page.evaluate(() => RW.game.prog.yard.ward || 0);
  console.log('towers after build', towers1, 'core after repair/armor', coreAfter, 'courtyard ward', yardLevel, 'errors', errors.length ? errors : 'none');
