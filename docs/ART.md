...
- **阶段 1 渲染器能力（Claude）**：卡通三段色阶、顶点色渐变、冷色边缘光、程序噪声材质、环境光遮蔽、限定到圣火/法术/敌眼的泛光、ACES、轻微暗角与夜间低雾。由 Claude 评估并实现；不让全屏泛光降低可读性，低画质需能关闭 AO 和描边。
- **阶段 3 UI 全套重做（Claude）**：深木框与金色描边的代码九宫格、统一金色描边图标、标题左侧竖排菜单、三层 HUD。提示不得覆盖画面中央 50%；将遮挡规则加入 `npm run audit:ui`。图标细目见 `docs/UI_ICON_SYSTEM_V1.md`，与 HUD、木框一并实现，避免单独重复改版。
- **阶段 2 美术基线（Codex）**：已按 A+ 圣经调整 `PAL`、昼夜色彩、英雄/敌人/建筑/塔造型；验收仍以改前改后截图、231×87 剪影和真机回执为准。
