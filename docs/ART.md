...
- [全游戏 UI 图标系统 V2](UI_ICON_SYSTEM_V2.md)：以上位《FG_ART_BIBLE_001》为准，落实圣经色票、16/24/32/48 网格、线宽/底板、双线索状态、夜战与中央战场禁入区；覆盖标题至结算、全部数据驱动内容、代码生成应用图标及交互状态。Canvas 图集实现、`audit:ui` 与日/夜战截图验收由 Claude 承接；除 A+ 允许的一款 OFL 子集字体外不新增素材。
