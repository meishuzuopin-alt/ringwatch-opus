...
### 手绘柔光试验（`grok/scene-painterly`，代码和着色器，没有新图片）

全部可调参数在 `RW.PAINTERLY`（`js/data.js`），`enabled: false` 时回到 C1。C1 的冷暖不改：圣火半径内仍是暖光，圈外仍是冷蓝。这一层只在那之上做四件事：

- **调色**：村落用低饱和的鼠尾草绿、苔绿、灰粉鲑色土路、哑青绿的水，光偏暖。别的地貌按 `biomeMix` 往这套颜色靠。圈外再降一档饱和，圈内更暖、饱和略高。
- **一个全屏通道**（接在色调映射之后、界面之前；界面是另一张画布）：轻量四象限 Kuwahara，邻域按 `kuwahara.step`（2）跳着采样，相当于半分辨率。立牌轮廓对比高的地方把滤镜混回去，角色和界面保持可读。同一遍里还有四道斜向的羽化光柱（高斯截面，宽 smoothstep，没有硬边，慢慢漂）和一圈羽化暗框，暗框边缘用低频噪声做出模糊的深色叶影，把视线收向亮着的广场。
- **光尘**：慢速漂的发光点，走原来的 `GL.glow`。颜色绿通道高于红蓝，吃到特效着色器里给绿萤火的压暗，亮度留在泛光阈值下面。
