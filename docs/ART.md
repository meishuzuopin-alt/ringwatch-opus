...
需求写在这里，格式：**需求 / 目的 / 验收标准**。Claude 做完后移到「已实现」并附截图说明。

- **整备页交互图像化（2026-09-24，负责人明确要求）**：武器、技能、建筑科技、道具等可购买选项必须配有可辨识的原创图像，不得继续做成纯文字卡；维修、加固、刷新、开始下一波等主要操作配清楚的小图标。完整问题记录、截图与验收标准见 [`docs/ART_REVIEW_WAVE_PREP_UI_2026-09-24.md`](ART_REVIEW_WAVE_PREP_UI_2026-09-24.md)。
...
- 不引入未授权的图片、模型文件、字体文件（见 `AGENTS.md` 硬规则 1）。负责人于 2026-09-24 明确批准为本次整备页交互增加原创/已授权图像；批准范围仅限 `docs/ART_REVIEW_WAVE_PREP_UI_2026-09-24.md` 指定的交互卡与操作图标，不扩展到场景模型、角色或其他页面。外部模型（如 GLB）仍须另行说明授权来源并先获负责人同意。
