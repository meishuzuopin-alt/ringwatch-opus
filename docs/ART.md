...
| 夜晚约为白天的 35% | 预设加 `amb` 倍率；夜晚月光、天光、雾色提亮成夜蓝 | `npm run light` 量出来是 37% |
...
**交给 Codex（阶段 2）**：昼夜预设里的新字段 `rim`、`fogLow`、`vig`、`amb` 可以直接调；夜晚和 Boss 的月光、天光、雾色为了达到 35% 亮度已经提亮过（夜晚雾色 `#34497a`），改色时跑一遍 `npm run light` 看比例。圣火光圈里的石板在夜里偏粉，是月光的淡紫加暖光混出来的，换石板颜色时一起看。英雄在 231×87 缩略图里还看不出来，要等阶段 2 放大到 48 像素以上。
...
- 全游戏图标设计见 [`docs/UI_ICON_SYSTEM_V1.md`](UI_ICON_SYSTEM_V1.md)。Claude 在阶段 3 与深木框、三层 HUD 一并实现；避免在阶段 3 前另行改 UI。
