...
- 横屏 16:9 桌面，镜头俯角约 46°。所有东西要在 **1280×720 的截图里一眼能读懂**：玩家、敌人、预警必须比地面显眼。运行时规范以 960×540 逻辑画布为准，评审截图是同一 16:9 画面整体放大 4/3 倍；不得建立第二套布局坐标。
...
- [全游戏 UI 图标系统 V2](UI_ICON_SYSTEM_V2.md)：以 `codex/art-direction-v1` 分支的 `docs/art/ART_DIRECTION_AND_HANDOFF_V1.md`（实际 handoff 路径，其中收录《FG_ART_BIBLE_001》）为准，落实圣经色票、16/24/32/48 网格、线宽/底板、双线索状态、夜战与中央战场禁入区；覆盖标题至结算、全部数据驱动内容、代码生成应用图标及交互状态。Canvas 图集实现、`audit:ui` 与日/夜战截图验收由 Claude 承接；除 A+ 允许的一款 OFL 子集字体外不新增素材。
