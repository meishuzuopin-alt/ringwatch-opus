...
### 场景贴图资产包（FG-TEX-001，2026-09-25；尚未接入运行时）

- **来源与权利**：全部贴图由本项目的 `tools/gen_textures.js` 原创、确定性生成，只使用 Node 内置模块；制作过程使用 AI 辅助编写生成算法，没有下载、导入或改编任何第三方图像与素材。发布 Steam 时在 AI 内容问卷中如实申报“AI 辅助制作的程序化场景贴图”。
- **范围**：4 类地面（512 主贴图 + 256 低配）、5 类建筑表面、白天 / 黄昏 / 夜晚天空、3 条卡通色带、4 张特效贴图，共 23 张 PNG；清单和 THREE 采样建议见 `assets/textures/manifest.json`。
- **风格与色表**：只用 FG-ART-001 色表及其明暗阶；草地基色主动降饱和。低频斑驳、稀疏笔触和大色块模拟水彩手作感，避免照片质感与高频噪点；危险圈固定使用敌眼红 `#FF3B3B`。
- **天空方案**：选择 1024×512 竖向渐变与低频云层合成，而不是 2048×1024 等距柱状图。960×540 目标下足够清晰，同时每张基础层显存减少约 75%；夜景维持深夜蓝，不抬整体曝光，给圣火暖光留出对比空间。
- **预算与复现**：RGBA8 贴图按 mipmap 额外 1/3 估算，总显存约 17.45 MiB，低于 P2000 的 24 MiB 项目预算。PNG 是忽略提交的生成产物；运行 `npm run gen:textures` 可逐字节复现全部资产，各文件 SHA-256 固定在 `manifest.json` 并由 `npm run check` 校验。`npm start` 和四个桌面打包命令也会先生成贴图。
- **预览**：`docs/art-upgrade/texture-preview.html` 以 3×3 展示所有贴图，并用 Three.js 提供白天 / 夜晚两个静态村庄机位；页面按钮可导出白天机位 PNG。
- **接入边界**：本阶段只交付资产和生成管线，未修改 `world3d.js`、`gl3d.js`、`render.js`、`sim.js`、`data.js`；运行时采样、材质与画质档接入留到下一阶段。
