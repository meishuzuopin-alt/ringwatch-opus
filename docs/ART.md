...
- **英雄至少 48 像素的自动断言**：防止镜头、模型或分辨率改动让主角失去可读性；在 960×540 战斗视图各镜头档位自动测量英雄屏幕包围盒高度，必须始终不少于 48 像素。
- **231×87 剪影自动出图**：固定检查英雄、普通敌人、精英、重甲与 Boss 的远景辨识度和比例层级；以透明底自动输出每类 231×87 像素剪影对照图，CI 上传供美术评审。
