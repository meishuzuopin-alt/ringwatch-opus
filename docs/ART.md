...
| 程序化手绘纹理 | 纯代码生成 256×256 无缝纹理（R 通道高频微噪、G 通道水彩涂抹、B 通道树冠光斑），0 外部资产依赖，注入着色器调制漫反射与日光 | `gl3d.js` 的 `buildProceduralTexture` 与 `uProcTex` |
| 蓝紫阴影保底 | 后处理调色通道中压制纯黑，暗部保底微染蓝紫色（`vec3(0.016, 0.020, 0.045)`），符合写实风格化暗部要求 | `gl3d.js` 的 `gradePass` |
