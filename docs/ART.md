...
- **A+ 方向：夜里唯一的暖光，粗轮廓、低多边形的绘本奇幻守村。** 造型胖、短、剪影清楚，远看像手工木雕玩具；代码生成，不照搬任何商业游戏的造型或配色。
- 横屏 16:9 桌面，镜头俯角约 46°。所有东西要在 **1280×720 的截图里一眼能读懂**：玩家、敌人、预警必须比地面显眼。
- **颜色遵循 FG_ART_BIBLE_001**：圣火金 `#FFB547`、余烬橙 `#E8702A`；暖土 `#C9A27A`、草地 `#6F9A4E`、屋顶赭红 `#A8483A`、木色 `#7A5234`；夜蓝 `#1E2A4A`、月青 `#5FA8C8`、敌紫 `#6B3FA0`、敌眼/攻击预警红 `#FF3B3B`。最亮暖色留给圣火与英雄，敌人身体保持冷紫；同屏高饱和色不超过三块。
- 昼夜节奏：第 1–3 波白天、第 4–6 波黄昏、第 7 波起入夜，Boss 波（每 5 波）加压。夜晚环境亮度目标约为白天 35%，夜蓝底色与圣火暖光圈分区清楚；不以全屏发光洗白画面。
...
- **阶段 1 渲染器能力（Claude）**：卡通三段色阶、顶点色渐变、冷色边缘光、程序噪声材质、环境光遮蔽、限定到圣火/法术/敌眼的泛光、ACES、轻微暗角与夜间低雾。避免全屏泛光；低画质保留关闭 AO 和描边的退路。
- **阶段 3 UI 全套重做（Claude）**：深木框与金色描边的代码九宫格、统一金色描边图标、三层 HUD；提示不得覆盖画面中央 50%，并把规则加入 UI 审计。图标细目见 `docs/UI_ICON_SYSTEM_V1.md`，与木框/HUD 一并接入。
- **阶段 2 美术基线（Codex）**：已在本分支按 A+ 圣经调整 `js/world3d.js` 的调色、昼夜环境、英雄/敌人/建筑与塔造型；待 CI 和截图核对，最终验收仍需 Grok 真机回执。
