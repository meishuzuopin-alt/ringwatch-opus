...
仓库里已有的只是应用图标（`assets/branding/icon_1024.png` 及 512 / 256 / 128），给 favicon、manifest 和加载画面用。它**不能**代替下面这些提交用封面和预览视频，那些还没做。

还缺的封面 / 缩略图（Game covers，三张都要，主体一致，游戏名写 Ringwatch）：

| 还缺 | 尺寸 | 比例 |
|---|---|---|
| 横图 Landscape | 1920×1080 | 16:9 |
| 竖图 Portrait | 800×1200 | 2:3 |
| 方图 Square | 800×800 | 1:1 |

还缺的预览视频（悬停缩略图，无声，≤ 50 MB，15–20 秒，更长会被裁到 20 秒）：

| 还缺 | 尺寸 | 比例 |
|---|---|---|
| 横屏预览 | 1920×1080（1080p） | 16:9 |
| 竖屏预览 | 1080×1620（1080p） | 2:3 |

封面限制：不要加边框；除游戏名外不要写 New / Play / Play now；不要放商店图标；不要用没有版权的图；不要糊。三张图的主体要能认成同一款游戏。

预览视频不要黑边、黑场加 logo、鼠标指针、Play Now、应用图标。不要自己加速，他们处理时会稍微加快。开头帧用对应的静态封面。
